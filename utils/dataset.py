...
if __name__ == "__main__":
    ...
    """
    Debugging area, don't panic
    """
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        '../../../hf/hub/facebook/wmt19-en-de')
    print('B1')
    d = DatasetForSeq2Seq('../../../data/translation/ed/i14',
                          'train', ('en', 'de'), max_length=512, ratio=1.5, tokenizer=tokenizer, num_workers=4)
    print('B2')
    c1 = DataCollector(tokenizer, d, 8192, 12, 0, 4)
    c2 = DataCollector(tokenizer, d, 8192, 12, 1, 4)
    print('Runing')
    import time
    import torch
    batches = 0
    tokens = 0
    sents = 0
    for i, (_c1, _c2) in enumerate(tqdm.tqdm(zip(c1, c2))):
        batches += 1
        sents += _c1['src']['input_ids'].shape[0] + _c2['src']['input_ids'].shape[0]
        _tokens = _c1['src']['input_ids'].numel() + _c2['src']['input_ids'].numel()
        tokens += _tokens
        # assert torch.equal(_c1['tgt']['input_ids'], _c2['tgt']['input_ids'])
    print(len(c1), len(c2))
    print(batches, sents)
    c1.cleanup()
    c2.cleanup()
